...
namespace MondoCore.Security.Encryption.UnitTests
{
    ...
    {
        [TestMethod]
        public async Task KeyStore_Add()
        {
            ...
            store.Setup( s=> s.Put(It.IsAny<string>(), It.IsAny<String>(), It.IsAny<Encoding?>()));
            await keyStore.Add(key);

            store.Verify( s=> s.Put(It.IsAny<string>(), It.IsAny<String>(), It.IsAny<Encoding?>()), Times.Once);
        }     
